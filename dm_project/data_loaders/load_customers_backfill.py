...
def load_data(*args, **kwargs):
    ...
    for (chunk_start_dt, chunk_end_dt) in _iter_day_windows(start_dt, end_dt, chunk_days):
        ...
        print(
            f"[customers] chunk {chunk_start_iso} → {chunk_end_iso} | rows={total_in_chunk} "
            f"| pages={page_number-1} | duration_s={duration_s}"
        )

    print(f"[customers] total_rows={len(all_rows)} for range {_iso_utc(start_dt)} → {_iso_utc(end_dt)}")
    return all_rows
